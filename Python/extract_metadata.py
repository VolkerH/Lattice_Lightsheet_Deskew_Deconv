...
def extract_lls_metadata(settingsfilepath: str, verbose: bool = False) -> pd.DataFrame:
    ...
    res["ZPZT"] = re.compile(
        "Z PZT Offset, .*\((?P<channel>\d+)\)\s*:\s*(?P<galvoscan_zoffset>[\d.]+)\s*(?P<galvoscan_interval>[\d.]+)\s*(?P<galvoscan_n_tiff>\d+)\s*"
    )
    # https://regex101.com/r/T0eS35/3
    res["SPZT"] = re.compile(
        "S PZT Offset, .*\((?P<channel>\d+)\)\s*:\s*(?P<samplescan_zoffset>[\d.]+)\s*(?P<dz_stage>[\d.]+)\s*(?P<samplescan_n_tiff>\d+)"
    )
    # https://regex101.com/r/pjsZXC/3
    res["Laser"] = re.compile(
        "Excitation Filter, Laser,.*\((?P<channel>\d+)\)\s+:\s+(?P<filter>.+)\s+(?P<lambda>[\d.]+)\s(?P<power>[\d.]+)\s(?P<exposure>[\d.]+).*"
    )
